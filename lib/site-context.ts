...
function toCategoryById(categories: readonly CategoryResponse[]): Readonly<Record<string, CategoryResponse>> {
  return categories.reduce<Record<string, CategoryResponse>>((accumulator, category) => {
    accumulator[category.id] = category;
    return accumulator;
  }, {});
}

function toNestedCategories(categories: readonly CategoryResponse[]): readonly CategoryResponse[] {
  const categoriesById = new Map(categories.map((category) => [category.id, { ...category, children: [] as CategoryResponse[] }]));
