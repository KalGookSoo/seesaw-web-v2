...
const environments = [
  {
    key: 'NEXT_PUBLIC_SEESAW_APPLICATION_NAME',
    value: APPLICATION_NAME,
    description: '현재 프로세스 환경변수에서 조회한 애플리케이션 이름'
  },
  {
    key: 'NEXT_PUBLIC_SEESAW_APPLICATION_VERSION',
  ...
  {
    key: 'NEXT_PUBLIC_APPLICATION_API_BASE_URL',
  ...
  },
  {
    ...
    value: `/manifest.json`,
    description:
      'seesaw-api에서 현재 사이트 정보로 동적 생성하는 PWA manifest URL'
  },
  {
      ...
      <section className="mx-auto max-w-5xl space-y-8">
        <div className="space-y-2">
          ...
          <p className="max-w-3xl text-base leading-7 text-zinc-600">
            현재 Next.js 프로세스에서 확인되는 애플리케이션 설정입니다.
            민감정보는 제외하고 애플리케이션 식별, 버전, 실행 환경, API 연결
            정보와 API 상수만 표시합니다.
          </p>
        </div>
            ...
            <tbody className="divide-y divide-zinc-200">
              {environments.map((environment) => (
                <tr key={environment.key}>
                  ...
                  <td className="px-5 py-4 font-mono text-xs break-all text-zinc-700">
                    {environment.value}
                  </td>
