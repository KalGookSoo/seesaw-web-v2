...
import { GlassmorphismAlert, GlassmorphismConfirm, GlassmorphismModal } from 'app/style-guide/glassmorphism/_components/feedback';
import { PageIntro, PreviewPanel, StyleGuideSection } from 'app/style-guide/glassmorphism/_components/style-guide-section';

const primaryButtonClassName =
  'rounded-full bg-[var(--glassmorphism-accent)] px-5 py-2.5 text-xs font-bold tracking-widest text-[var(--glassmorphism-accent-contrast)] uppercase shadow-lg shadow-[var(--glassmorphism-accent)]/20 transition-colors hover:bg-[var(--glassmorphism-accent-muted)]';
...
const descriptionClassName = 'text-sm leading-6 text-[var(--glassmorphism-ink-soft)]';
const inputClassName =
  'h-11 w-full rounded-full border border-[var(--glassmorphism-line)] bg-[var(--glassmorphism-surface-muted)] px-4 font-mono text-xs text-[var(--glassmorphism-ink)] outline-none backdrop-blur-xl focus:border-[var(--glassmorphism-accent)]';
    ...
    <>
      ...
      <StyleGuideSection eyebrow="Alert" title="Alert variants">
        <div className="grid gap-6 lg:grid-cols-2">
          <GlassmorphismAlert title="정보" variant="info">
            새 댓글 알림을 받을 수 있도록 브라우저 알림 권한을 허용해주세요.
          </GlassmorphismAlert>
          <GlassmorphismAlert title="성공" variant="success">
            캘린더 구독 URL이 클립보드에 복사되었습니다.
          </GlassmorphismAlert>
          <GlassmorphismAlert title="주의" variant="warning">
            현재 카테고리는 검색엔진 노출 대상이 아닙니다.
          </GlassmorphismAlert>
          <GlassmorphismAlert title="오류" variant="danger">
            요청 값을 다시 확인해주세요. UUID v4 형식이 아닙니다.
          </GlassmorphismAlert>
          <GlassmorphismAlert variant="neutral">제목 없이 짧은 안내만 전달할 때 사용하는 neutral alert입니다.</GlassmorphismAlert>
        </div>
      </StyleGuideSection>
      ...
      <StyleGuideSection eyebrow="Modal" title="Modal sizes and actions">
        ...
        >
          <div className="grid gap-4 lg:grid-cols-2">
            <div className="rounded-2xl border border-[var(--glassmorphism-line-soft)] bg-[var(--glassmorphism-surface-muted)] p-6 backdrop-blur-xl">
              <p className="font-sans text-xs font-bold tracking-wider text-[var(--glassmorphism-ink)] uppercase">제목</p>
              <p className="mt-2 text-sm text-[var(--glassmorphism-ink-soft)]">7월 정기 독서 모임</p>
            </div>
            <div className="rounded-2xl border border-[var(--glassmorphism-line-soft)] bg-[var(--glassmorphism-surface-muted)] p-6 backdrop-blur-xl">
              <p className="font-sans text-xs font-bold tracking-wider text-[var(--glassmorphism-ink)] uppercase">장소</p>
              <p className="mt-2 text-sm text-[var(--glassmorphism-ink-soft)]">대전광역시 중구 중앙로</p>
            </div>
          </div>
        </GlassmorphismModal>
      </StyleGuideSection>
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          ...
          <PreviewPanel title="Danger confirm">
            <div className="space-y-4">
              ...
              <button
                className="rounded-full bg-[var(--glassmorphism-danger)] px-5 py-2.5 text-xs font-bold tracking-widest text-white uppercase shadow-[var(--glassmorphism-danger)]/20 shadow-lg transition-colors hover:brightness-110"
                type="button"
