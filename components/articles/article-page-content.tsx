...
import { ChevronLeft, ChevronRight, Eye, Grid2X2, Inbox, List, MessageCircle, Pencil, Pin, Search, UserRound } from 'lucide-react';

import type { ArticleResponse, CategoryResponse, PagedModel } from '@/types/site';
...
function buildArticlesHref(search: SearchArticlesParams, overrides: Partial<SearchArticlesParams>): string {
...
function ArticleMeta({ article, showViews = false }: Readonly<{ article: ArticleResponse; showViews?: boolean }>) {
  return (
...
function ViewSwitcher({ search, viewType }: Readonly<{ search: SearchArticlesParams; viewType: ArticleViewType }>) {
  return (
...
function SearchForm({ search }: Readonly<{ search: SearchArticlesParams }>) {
  return (
    <form action="/articles" className={`${mattePanelClassName} grid gap-2 p-2 sm:grid-cols-[10rem_1fr_auto]`}>
      <input name="categoryId" type="hidden" value={search.categoryId} />
      {search.categoryType ? <input name="categoryType" type="hidden" value={search.categoryType} /> : null}
      ...
      <input
        ...
        aria-label="검색어"
      />
      <button className={`${controlClassName} bg-default-blue px-4 font-semibold text-white hover:brightness-95`} type="submit">
...
function EmptyState() {
  return (
    <div className={`${mattePanelClassName} px-5 py-14 text-center`}>
      ...
      <p className="text-default-label mt-4 text-base font-medium">게시물이 없습니다.</p>
      <p className="text-default-secondary-label mt-2 text-sm">새로운 글이 올라오면 이곳에 표시됩니다.</p>
    </div>
  );
}
      ...
      <div className="min-w-0">
        ...
        >
          ...
          {isRecentlyGenerated(article.createdDate) ? (
            <span className="bg-default-blue-soft text-default-blue-contrast rounded-full px-2 py-0.5 text-[10px] font-bold">NEW</span>
          ) : null}
        </Link>
  ...
  return (
    <section className={`${mattePanelClassName} overflow-hidden`}>
      <header className="border-default-separator/70 bg-default-fill/55 flex items-center justify-between border-b px-4 py-3">
        ...
        <span className="text-default-tertiary-label text-xs">총 {page.page?.totalElements ?? page.content.length}개</span>
      </header>
      <ul>
        {fixedArticles.map((article) => (
          <TableArticleRow key={article.id} article={article} indexLabel="공지" pinned />
        ))}
        {articles.map((article, index) => (
          <TableArticleRow
            key={article.id}
            article={article}
            indexLabel={String(Math.max(totalElements - pageNumber * pageSize - index, 1))}
          />
        ))}
      </ul>
    </section>
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-1 flex-col p-4">
        <h2 className="text-default-label group-hover:text-default-blue line-clamp-2 text-base font-medium">{article.title}</h2>
        <p className="text-default-secondary-label mt-3 line-clamp-3 flex-1 text-sm leading-6">{content || '본문 미리보기가 없습니다.'}</p>
        <ArticleMeta article={article} />
      </div>
    </Link>
  );
...
function ArticleCards({ page }: Readonly<{ page: PagedModel<ArticleResponse> }>) {
...
function Pagination({ page, search }: Readonly<{ page: PagedModel<ArticleResponse>; search: SearchArticlesParams }>) {
  ...
  return (
    ...
    >
      <p className="text-default-secondary-label text-sm">
        총 <span className="text-default-label font-semibold">{totalElements}</span>개 · {current + 1} / {totalPages}
      </p>
      <div className="flex items-center justify-center gap-1">
        ...
        <Link
          ...
          href={buildArticlesHref(search, { page: Math.min(current + 1, totalPages - 1) })}
          aria-label="다음 페이지"
        >
...
export function StaticContentPage({ articles, category }: StaticContentPageProps) {
  return (
    <main className="bg-default-surface-grouped min-h-full px-4 py-6 sm:px-6">
      <header className={`${mattePanelClassName} mx-auto mb-5 max-w-4xl px-5 py-5`}>
        <p className="text-default-secondary-label text-sm font-medium">Static content</p>
        <h1 className="text-default-label mt-2 text-3xl font-semibold">{category.name}</h1>
        {category.description ? <p className="text-default-secondary-label mt-3 text-base leading-7">{category.description}</p> : null}
      </header>
...
export function ArticlePageContent({ category, fixedArticles, page, search, viewType }: ArticlePageContentProps) {
  return (
    <main className="bg-default-surface-grouped min-h-full px-4 py-6 sm:px-6">
      <div className="mx-auto max-w-6xl space-y-5">
        <header className={mattePanelClassName}>
          <div className="flex flex-col gap-4 px-5 py-5 sm:flex-row sm:items-end sm:justify-between">
            <div className="min-w-0">
              <p className="text-default-secondary-label text-sm font-medium">{category.type ?? 'BOARD'}</p>
              <h1 className="text-default-label mt-2 truncate text-3xl font-semibold">{category.name}</h1>
              {category.description ? <p className="text-default-secondary-label mt-3 text-base leading-7">{category.description}</p> : null}
            </div>
          ...
          <div className="border-default-separator/70 bg-default-fill/40 flex flex-col gap-3 border-t px-5 py-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex flex-wrap items-center gap-2">
              <Link
                ...
                href={buildArticlesHref(search, { keyField: 'createdBy', keyWord: '', page: 0 })}
              >
        ...
        {viewType === 'CARD' ? <ArticleCards page={page} /> : <ArticleTable fixedArticles={fixedArticles} page={page} />}
